...
app.get('/', function (request, response) {
    Message.find({}).sort('-createdAt').exec(function (err, data) {
...
app.post('/comment', function (request, response) {
    Comment.create({ content: request.body.cmnt, name: request.body.name }, function (err, data) {
        if (err) {
            console.log("Something went wrong!:", err);
            for (var key in err.errors) {
                request.flash('registration', err.errors[key].message);
            ...
            Message.findOneAndUpdate({ _id: request.body.message }, { $push: { comments: data } }, function (err) {
                if (err) {
                    console.log("Something went wrong when pushing the comment into the message!:", err);
                    for (var key in err.errors) {
                        request.flash('registration', err.errors[key].message);
                    }
                    response.redirect('/');
